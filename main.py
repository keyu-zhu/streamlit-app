...
avatar = {"user": None, "assistant": config["streamlit"]["avatar"]}
...
example_prompt: str = st.sidebar.selectbox(
    label="Example Questions", options=EXAMPLE_QUESTIONS, key="example_question"
)
...
logo_slot, title_slot = st.columns([0.6, 20])  # Adjust the ratio as needed
...
with title_slot:
    st.markdown(
        f'<h1 class="no-padding-margin-border">{title}</h1>', unsafe_allow_html=True
    )
